...
public class Controller : IMessageHandler
{
	...
	public void onMessage(Message msg)
	{
		...
		try
		{
			...
			//	Res.outz(string.Format("cmd 92: {0}|{1}|{2}|{3}|{4}|{5}|{6}|{7}|isChatServer={8}",text4,str2,char7.charID,char7.head,char7.headICON,char7.body,char7.bag,char7.leg,b45));
				empty += str2;
				InfoDlg.hide();
				if (text4.Equals(string.Empty))
				{
			...
			case -5:
			{
				...
				// // Debug.Log("Cmd: -5 PLAYER_ADD: cID| cName| cFlag| cBag|    " + @char.charID + " | " + @char.cName + " | " + @char.cFlag + " | " + @char.bag);
				break;
			}
			case -7:
			{
		...
		try
		{
			...
			case 0:
			{
				...
				for (int num17 = 0; num17 < num16; num17++)
				{
                    ...
                    /*        using (StreamWriter writer = new StreamWriter("D:\\ServerGame\\NroFull\\data\\vinh\\head_avatar.txt",true))
                            {
                                writer.WriteLine(Char.idHead[num17]+"|"+ Char.idAvatar[num17]);
                            }*/
                }
	...
	private void loadItemNew(myReader d, sbyte type, bool isSave)
	{
		try
		{
			...
			else if (type == 2)
			{
				...
				Res.outz("ITEM2 Start = " + num2);
                Res.outz("ITEM2 Count = " + num3);
