...
public class GameCanvas : IActionListener
{
	...
	public void initGame()
	{
		...
		isHighFps = Rms.loadRMSInt("fps") == 1;
	...
	public static void paintBGGameScr(mGraphics g)
	{
		...
		int gH = GameScr.gH;
		g.translate(-g.getTranslateX(), -g.getTranslateY());
        if (GameMod.XoaBackground)
        {
            g.setColor(GameMod.GetColor());
            g.fillRect(0, 0, w, h);
            return;
        }
        try
    ...
    {
        if ((info.ToLower().Contains("giây") || info.ToLower().Contains("đủ") || info.ToLower().Contains("không") || info.ToLower().Contains("không thể đến") || info.ToLower().Contains("cần")) && (GameCanvas.currentScreen == GameScr.instance))
        {
