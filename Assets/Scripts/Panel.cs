...
public class Panel : IActionListener, IChatable
{
	...
	private void updateKeyScrollView()
	{
		...
		bool flag = false;
        if (this.type == 3 && GameCanvas.isPointerHoldIn(this.wScroll - 35, this.yScroll, 35, this.hScroll))
        {
            GameCanvas.clearAllPointerEvent();
            if (!GameMod.isVaoKhu)
            {
				GameMod.StartVaoKhu((this.cmtoY + GameCanvas.py - this.yScroll) / this.ITEM_HEIGHT, TileMap.zoneID, TileMap.mapID);
                return;
            }
            GameMod.isVaoKhu = false;
	...
	private void paintZone(mGraphics g)
	{
		...
		for (int i = 0; i < pts.Length; i++)
		{
            ...
            g.fillRect(this.wScroll - 34, y, 36, h2, 2721889, 90);
            mFont.tahoma_7.drawString(g, GameMod.isVaoKhu ? "Hủy" : "Auto", this.wScroll - 15, num2 + 6, 3);
        }
		paintScrollArrow(g);
	}
	...
	private void paintInventory(mGraphics g)
	{
		...
		try
		{
			for (int j = 1; j < currentListLength; j++)
			{
				...
				mFont2.drawString(g, item.template.name + text, num4 + 5, num5 + 1, 0);
				string text2 = string.Empty;
				if (item.itemOption != null)
				{
					if (item.itemOption.Length > 0 && item.itemOption[0] != null && item.itemOption[0].optionTemplate.id != 102 && item.itemOption[0].optionTemplate.id != 107)
	...
	private void doFirePetStatus()
	{
		...
		if (selected < 4)
        {
            GameMod.petStatus = (sbyte)selected;
	...
	private void doFireCombine()
	{
		if (currentTabIndex == 0)
		{
            ...
            if (GameMod.isDapDo)
            {
                myVector.addElement(new Command("Chọn Số Sao Cần Đập", this, 8010, currItem));
            }
            if (currItem != null)
			{
				GameCanvas.menu.startAt(myVector, X, (selected + 1) * ITEM_HEIGHT - cmy + yScroll);
	...
	public void perform(int idAction, object p)
	{
        ...
        if (idAction == 8010)
        {
            VuDangChatTextField("Nhập Số Sao Cần Đập", "Chỉ Được Nhập Số");
        }
        if (idAction == 8007)
        {
            string text = (string)p;
            GameMod.tennoitaicanmo = text.Substring(0, text.IndexOf('%') - 3);
            GameCanvas.panel.hide();
            VuDangChatTextField("Nhập Chỉ Số Nội Tại", "Chỉ Được Nhập Số (% Nội Tại Cần Mở)");
        }
        if (idAction == 8001)
		{
	...
	public void onChatFromMe(string text, string to)
	{
        ...
        if (chatTField.strChat == "Nhập Số Sao Cần Đập")
        {
            GameMod.soSaoCanDap = int.Parse(text);
            Char.myCharz().addInfo("Đập " + GameMod.doDeDap.template.name + " Đến " + GameMod.soSaoCanDap + " Sao");
            chatTField.isShow = false;
            return;
        }
        if (chatTField.strChat == "Nhập Chỉ Số Nội Tại")
        {
            GameMod.ntMin = int.Parse(text);
            Char.myCharz().addInfo("Dừng khi chỉ số của nội tại đã chọn >= " + GameMod.ntMin + "%");
            chatTField.isShow = false;
			return;
        }
