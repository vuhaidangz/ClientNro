...
public class GameScr : mScreen, IChatable
{
	...
	public static void loadBg()
	{
        ...
        imgLogo = GameCanvas.loadImage("/mainImage/logo1.png");
			...
			if (GameCanvas.keyAsciiPress != 0)
			{
				if (mobCapcha == null)
				{
					if (TField.isQwerty)
					{
						...
						GameMod.PhimTat(GameCanvas.keyAsciiPress);
					}
	...
	private void updateKeyTouchControl()
	{
		...
		else if (isHaveSelectSkill)
		{
            ...
            {
                int num = 95;
                for (int a = 0; a < GameMod.chars2.Length; a++)
                {
                    if (GameMod.chars2[a] != null)
                    {
                        if (GameCanvas.isPointerHoldIn(GameCanvas.w - 155, num, 150, 10))
                        {
                            GameMod.GotoXY(GameMod.chars2[a].cx, GameMod.chars2[a].cy);
                            Char.myCharz().focusManualTo(GameMod.chars2[a]);
                            Char.myCharz().currentMovePoint = null;
                            GameCanvas.clearAllPointerEvent();
                        }
                        num += 10;
            ...
            if (GameMod.thongBaoBoss)
            {
                for (int i = 0; i < GameMod.bossVip.size(); i++)
                {
                    if (GameCanvas.isPointerHoldIn(GameCanvas.w - 23, 35 + 10 * i, 28, 9))
                    {
                        GameCanvas.isPointerJustDown = false;
                        isPointerDowning = false;
                        if (GameCanvas.isPointerClick && GameCanvas.isPointerJustRelease)
                        {
                            ...
                            {
                                XmapController.StartRunToMapId(GameMod.GetIDMap(((ShowBoss)GameMod.bossVip.elementAt(i)).mapName));
                            }
                            else
                            {
	...
	public override void paint(mGraphics g)
	{
		...
		GameMod.Paint(g);
		paint_xp_bar(g);
        //mFont.bigNumber_red.drawString(g, "NamZ Đẹp Chai", 250, 30, mFont.LEFT);
        int screenWidth = GameCanvas.w;

        // Lấy chiều rộng của hình ảnh
        int logoWidth = imgLogo.getWidth(); // Sử dụng phương thức thích hợp của hình ảnh

        // Tính toán tọa độ X để vẽ hình ảnh chính giữa màn hình
        int logoX = (screenWidth - logoWidth) / 2;

        // Vẽ hình ảnh chính giữa màn hình
        g.drawImage(imgLogo, logoX, 2, 0);
        if (!isPaintOther)
		{
	...
	private void paintTouchControl(mGraphics g)
	{
		...
		if (!TileMap.isOfflineMap() && !isVS())
		{
			...
			{
				g.drawImage((!Main.isPC || Main.isAndroid) ? imgChat2 : imgChatsPC2, xC + 17, yC + 17 + mGraphics.addYWhenOpenKeyBoard, mGraphics.HCENTER | mGraphics.VCENTER);
			}
			else
			{
				g.drawImage((!Main.isPC || Main.isAndroid) ? imgChat : imgChatPC, xC + 17, yC + 17 + mGraphics.addYWhenOpenKeyBoard, mGraphics.HCENTER | mGraphics.VCENTER);
			}
		}
		if (isUseTouch)
		{
	...
	private void paintGamePad(mGraphics g)
	{
		if (isAnalog != 0 && Char.myCharz().statusMe != 14)
		{
			g.drawImage((mScreen.keyTouch != 5 ) ? imgFire0 : imgFire1, xF + 20, yF + 20, mGraphics.HCENTER | mGraphics.VCENTER);
			gamePad.paint(g);
	...
	public void chatVip(string chatVip)
	{
        ...
        {
            GameMod.bossVip.addElement(new ShowBoss(chatVip));
            if (GameMod.bossVip.size() > 5)
            {
                GameMod.bossVip.removeElementAt(0);
            }
        }
    }
